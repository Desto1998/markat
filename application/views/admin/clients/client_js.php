...
$(function() {
    ...
    $("#new_ansprechpartner").on("hidden.bs.modal", function () {
        $('#new_ansprechpartner form').trigger("reset");
        $('#new_ansprechpartner h4 span').html('Erstellen ');
    });

    $('#ansprechpartner').on('click', '._edit', function (event) {
        event.preventDefault();
        var dd = $(this).data('id');
        $('#new_ansprechpartner h4 span').html('Bearbeiten ');
        requestGet('clients/get_ansprechpartner/' + dd).done(function (response) {
            response = JSON.parse(response);
            $('#newoccupation #ansprechpartner_id').val(response.id);
            $('#new_ansprechpartner #vorname').val(response.vorname);
            $('#new_ansprechpartner #nachname').val(response.nachname);
            $('#new_ansprechpartner #position').val(response.position);
            $('#new_ansprechpartner #email').val(response.email);
            $('#new_ansprechpartner #telefon').val(response.telefon);
            $('#new_ansprechpartner #notizen').val(response.notizen);
            $('#new_ansprechpartner').modal('show');
        });
    });
    ...
    $('a[href="#customer_admins"], a[href="#ansprechpartner"]').on('click', function() {
        $('.btn-bottom-toolbar').addClass('hide');
    });

    $('.profile-tabs a').not('a[href="#customer_admins"], a[href="#ansprechpartner"]').on('click', function() {
