...
$aColumns = array_merge($aColumns, [
    ...
    db_prefix() . 'contacts.email as email','1',
...
foreach ($rResult as $aRow) {
    ...
    $row[] = $rowName;
    $row[] = $aRow['lastname'];
    $row[] = "<div class='text-center'>" . count($this->ci->mieter_model->get_betreuer_by($aRow['id'])) . "</div>";
    ...
    if ($aRow['userid'] != 0) {
        $client = $this->ci->clients_model->get($aRow['userid']);
        $row[] = '<a href="' . admin_url('clients/client/' . $aRow['userid']) . '">' . $client->company . '</a>';
    } else {
        $row[] = '';
    }
